...
import Database.Orville.Oracle.ODBC
...
main = do
  ...
  _ <- O.runOrville initialInsertStudents env
    ...
    (\e -> do
       ...
       putStrLn
         ("Warning: Could not insert record. ODBC error message:\n\"" ++
          err ++ "\"\n"))
  ...
  putStrLn $
    ...
    (\e -> do
       let err = show (e :: SomeException)
       putStrLn
         ("Warning: Could not delete record. ODBC error message:\n\"" ++
          err ++ "\"\n"))
  catch
...
initialInsertStudents :: O.OrvilleT ODBC.Connection IO ()
...
deleteMajorDoesNotExist :: O.OrvilleT ODBC.Connection IO (Integer)
...
violateFKDelete :: O.OrvilleT ODBC.Connection IO (Integer)
violateFKDelete = do
  let condit = [(O..==) majorIdField (MajorId 2)]
  O.deleteWhere majorTable condit

findRecordsTest ::
     O.OrvilleT ODBC.Connection IO (Map.Map StudentId (Student StudentId))
findRecordsTest = do
...
updateFieldsTest :: O.OrvilleT ODBC.Connection IO (Integer)
updateFieldsTest = do
...
popRecordTest :: O.OrvilleT ODBC.Connection IO (Student StudentId)
popRecordTest = do
  let poppedRecord = O.popRecord' studentTable (StudentId 1)
  O.popThrow poppedRecord ()

popHasOneTest :: O.OrvilleT ODBC.Connection IO (Student StudentId)
popHasOneTest = do
  let hasOneRec = O.hasOne' studentTable studentMajorField
  O.popThrow hasOneRec (MajorId 3)

popHasManyTest :: O.OrvilleT ODBC.Connection IO [Student StudentId]
popHasManyTest = do
  let manyRecs = O.hasMany studentTable studentMajorField
  O.popThrow manyRecs (MajorId 3)

popManyHasOne :: O.OrvilleT ODBC.Connection IO [Student StudentId]
popManyHasOne = do
...
popManyHasMany :: O.OrvilleT ODBC.Connection IO [[Student StudentId]]
popManyHasMany = do
...
runAllMajors :: O.OrvilleT ODBC.Connection IO [StudentsForMajor]
