...
-}
mkCreateTableStep :: Orville.TableDefinition key writeEntity readEntity
                  -> MigrationStep
mkCreateTableStep =
...
-}
mkAlterTableStep :: Set.Set IS.ColumnName
                 -> Orville.TableDefinition key writeEntity readEntity
                 -> Maybe MigrationStep
mkAlterTableStep existingColumns tableDef =
  let
    addFieldAction :: Orville.FieldDefinition nullability a
                   -> accessor
...
-}
mkAlterTableAction :: Set.Set IS.ColumnName
...
mkAlterTableAction existingColumns fieldDef =
  ...
  in
    if
      Set.member isColumnName existingColumns
    then
      Nothing
    else
